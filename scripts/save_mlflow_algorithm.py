...
ALGORITHM_JSON_PATH = os.getenv("ALGORITHM_JSON_PATH", "../src/assets/default_models.json")
...
def migrate_all_algorithms(overwrite=False, tracking_uri=None, username=None, password=None, json_path=None):
    ...
    for algorithm in algorithms:
        try:
            ...
            if result["status"] == "success":
                logger.info(f"Migrated {algorithm['model_name']} (version {result['version']})")
                success_count += 1
            elif result["status"] == "exists":
                logger.info(f"Skipped {algorithm['model_name']} (already exists)")
                skip_count += 1
            else:
                logger.warning(f"Failed to migrate {algorithm['model_name']}: {result.get('error')}")
        except Exception as e:
            logger.error(f"Error migrating {algorithm.get('model_name', 'unknown')}: {e}")
...
def list_algorithms(tracking_uri=None, username=None, password=None):
    ...
    for name in client.algorithm_names:
        ...
        logger.info(f"  - {name} (v{algorithm.get('version', 'unknown')}): {algorithm.get('description', 'No description')}")
